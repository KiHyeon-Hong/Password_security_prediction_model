...
for (let i = 0; i < data.length; i++) {
    for (let j = 0; j < 10; j++) {
        // 취약한 부분 찾기
        var zxcvbnPoint =
            parseInt(parseInt(koreanZxcvbn(data[0]).score * 2 + comparePoint.frequencyComparePoint(data[0])) / 2) < 5
                ? parseInt(parseInt(koreanZxcvbn(data[0]).score * 2 + comparePoint.frequencyComparePoint(data[0])) / 2)
                : 4;
