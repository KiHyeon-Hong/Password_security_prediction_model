...
async function passwordValidation() {
    ...
    for(let i = 0; i < validationResult.length; i++) {
        fs.appendFileSync(__dirname + '/../files/updateValidationResult.txt', validationString[i] + ',' + validationData[i][0] + ',' + validationData[i][1] + ',' + validationData[i][2] + ',' + validationLeak[i] + '\n', 'utf8');
    }
    ...
    for(let i = 0; i < validationResult.length; i++) {
        if(validationResult[i] < checkPoint)
            fs.appendFileSync(__dirname + '/../files/updateLeakPredictPassword.txt', validationString[i] + '\n', 'utf8');
    }
    ...
    for(let i = 0; i < validationResult.length; i++) {
        if((validationResult[i] > checkPoint && validationAnswer[i] > checkPoint) || (validationResult[i] <= checkPoint && validationAnswer[i] <= checkPoint)) {
            ...
            noGood++;
            if(validationResult[i] > checkPoint)
                fs.appendFileSync(__dirname + '/../files/updateErrorResultLeak.txt', validationString[i] + ',' + validationAnswer[i] + ',' + validationResult[i] + ',' + validationLeak[i] + '\n', 'utf8');
            else
                fs.appendFileSync(__dirname + '/../files/updateErrorResultNotLeak.txt', validationString[i] + ',' + validationAnswer[i] + ',' + validationResult[i] + ',' + validationLeak[i] + '\n', 'utf8');
        }
    }
